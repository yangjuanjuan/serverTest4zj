...
public class ExecuteMetricsTest {
    private static final Logger log= LoggerFactory.getLogger(ExecuteMetricsTest.class);
    private GraphAnalysisClientApi graphAnalysisClient;
    private List<Map<String, String>> executeMetricsParams;
    ...
    public void setUp(){
        ...
        executeMetricsParams = ReadExcelUtil.getExcelList(GraphAnalysisClientApi.CASE_FILE,GraphAnalysisClientApi.EXECUTE_METRICS);
...
//    通过读取Excel获取测试数据Request Parameter
    @DataProvider
    public Object[][] executeMetricsParams(){
        Object[][] files = new Object[executeMetricsParams.size()][];
        for(int i=0; i<executeMetricsParams.size(); i++){
            files[i] = new Object[]{executeMetricsParams.get(i)};
        }
        return files;
    }
    @Test(dataProvider = "executeMetricsParams")
    public void executeMetricsTest(Map<?,?> param) throws IOException, InterruptedException {
...
//            params="{\"projectId\":"+proId+",\"graphId\":"+graphId+"}";
            ...
            CloseableHttpResponse re = graphAnalysisClient.executeMetrics(params);
