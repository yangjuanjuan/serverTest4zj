...
public class QueryPageRankTest {
    private static final Logger log= LoggerFactory.getLogger(QueryPageRankTest.class);
    private GraphAnalysisClientApi graphAnalysisClient;
    private List<Map<String, String>> queryPageRankParams;
    ...
    public void setUp(){
        ...
        queryPageRankParams = ReadExcelUtil.getExcuteList("queryPageRank.xlsx");
...
//    通过读取Excel获取测试数据Request Parameter
    @DataProvider
    public Object[][] queryPageRankParams(){
        Object[][] files = new Object[queryPageRankParams.size()][];
        for(int i=0; i<queryPageRankParams.size(); i++){
            files[i] = new Object[]{queryPageRankParams.get(i)};
        }
        return files;
    }
    @Test(dataProvider = "queryPageRankParams")
    public void queryPageRankTest(Map<?,?> param) throws IOException, InterruptedException {
...
//            params="{\"projectId\":"+proId+",\"graphId\":"+graphId+"}";
            ...
            CloseableHttpResponse re = graphAnalysisClient.queryPageRank(params);
