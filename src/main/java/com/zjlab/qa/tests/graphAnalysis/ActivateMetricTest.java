...
public class ActivateMetricTest {
    private static final Logger log= LoggerFactory.getLogger(ActivateMetricTest.class);
    private GraphAnalysisClientApi graphAnalysisClient;
    private List<Map<String, String>> activateMetricParams;
    ...
    public void setUp(){
        ...
        activateMetricParams = ReadExcelUtil.getExcuteList("activateMetric.xlsx");
...
//    通过读取Excel获取测试数据Request Parameter
    @DataProvider
    public Object[][] activateMetricParams(){
        Object[][] files = new Object[activateMetricParams.size()][];
        for(int i=0; i<activateMetricParams.size(); i++){
            files[i] = new Object[]{activateMetricParams.get(i)};
        }
        return files;
    }
    @Test(dataProvider = "activateMetricParams")
    public void activateMetricTest(Map<?,?> param) throws IOException, InterruptedException {
...
//            params="{\"projectId\":"+proId+",\"graphId\":"+graphId+"}";
            ...
            CloseableHttpResponse re = graphAnalysisClient.activateMetric(params);
