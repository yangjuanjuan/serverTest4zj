...
public class QueryAttrStyleTest {
    private static final Logger log= LoggerFactory.getLogger(QueryAttrStyleTest.class);
    private GraphAnalysisClientApi graphAnalysisClient;
    private List<Map<String, String>> queryAttrStyleParams;
    ...
    public void setUp(){
        ...
        queryAttrStyleParams = ReadExcelUtil.getExcuteList("queryAttrStyle.xlsx");
...
//    通过读取Excel获取测试数据Request Parameter
    @DataProvider
    public Object[][] queryAttrStyleParams(){
        Object[][] files = new Object[queryAttrStyleParams.size()][];
        for(int i=0; i<queryAttrStyleParams.size(); i++){
            files[i] = new Object[]{queryAttrStyleParams.get(i)};
        }
        return files;
    }
    @Test(dataProvider = "queryAttrStyleParams")
    public void queryAttrStyleTest(Map<?,?> param) throws IOException , InterruptedException{
...
//                查询视图数据，获取categoryIds
                ...
                categoryIds.add(GetJsonValueUtil.getValueByJpath(queryReJson, "result/categories[0]/id"));
...
//            params="{\"projectId\":"+proId+",\"graphId\":"+graphId+"}";
            ...
            CloseableHttpResponse re = graphAnalysisClient.queryAttrStyle(params);
