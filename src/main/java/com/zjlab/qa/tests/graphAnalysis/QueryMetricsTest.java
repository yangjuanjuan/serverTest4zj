...
public class QueryMetricsTest {
    private static final Logger log= LoggerFactory.getLogger(QueryMetricsTest.class);
    private GraphAnalysisClientApi graphAnalysisClient;
    private List<Map<String, String>> queryMetricsParams;
    ...
    public void setUp(){
        ...
        queryMetricsParams = ReadExcelUtil.getExcuteList("queryMetrics.xlsx");
...
//    通过读取Excel获取测试数据Request Parameter
    @DataProvider
    public Object[][] queryMetricsParams(){
        Object[][] files = new Object[queryMetricsParams.size()][];
        for(int i=0; i<queryMetricsParams.size(); i++){
            files[i] = new Object[]{queryMetricsParams.get(i)};
        }
        return files;
    }
    @Test(dataProvider = "queryMetricsParams")
    public void queryMetricsTest(Map<?,?> param) throws IOException, InterruptedException {
...
//            params="{\"projectId\":"+proId+",\"graphId\":"+graphId+"}";
            ...
            String exeParmas="{\"projectId\":"+proId+",\"graphId\":"+graphId+",\"type\":\"node\""+"}";
            graphAnalysisClient.executeMetrics(exeParmas);

            CloseableHttpResponse re = graphAnalysisClient.queryMetrics(params);
